...
The reason 'Identity' is used here is for 'ASetter' to be composable with other types, such as 'Lens'.Lens
Technically, if you're writing a library, you shouldn't use this type for setters you are exporting from your librarry; the right type to use is @<http://hackage.haskell.org/package/lens/docs/Control-Lens-Setter.html#t:Setter Setter>@, but it is not provided by this package (because then it'd have to depend on <http://hackage.haskell.org/package/distributive distributive>). It's completely alright, however, to export functions which take an 'ASetter' as an argument.
-}
...
{- |
A @SimpleGetter s a@ extracts @a@ from @s@; so, it's the same thins as @(s -> a)@, but you can use it in lens chains because its type looks like this:
...
-- 'Lens.Micro.Extras.view' :: 'Getter' a s a -> s - a
...
{- |
@Lens s t a b@ is the lowerst common denominator of a setter and a getter, something that has the power of both; it has a 'Functor' constraint, and since both 'Const' and 'Identity' are functors, it can be used whenever a getter or a setter is needed.
...
{- |
@Traversal s t a b@ is a generalisation of 'Lens' which allows many targets (possibly 0). It's achieved by changing the constraint to 'Applicative' instead of 'Functor' - indeeed, the point of 'Applicative' is that you can combine effects, which is just what we need to have many targets.
...
Traversing any value twice is a violation of traversal laws. You can, however, travers values in any order.
-}
